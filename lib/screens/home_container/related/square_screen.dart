...
import 'package:worldsocialintegrationapp/models/square_feed_model.dart';
...
class _SquareScreenState extends State<SquareScreen> {
  List<SquareFeedModel> momentsList = [];
  ...
  void loadMoments() async {
    ...
    await apiCallProvider.postRequest(API.getAllUserPost, reqBody).then(
      (value) {
        if (value['details'] != null) {
          for (Map<String, dynamic> item in value['details']) {
            momentsList.add(SquareFeedModel.fromJson(item));
          }
          setState(() {});
        }
      },
  ...
  Container getMomentCard(SquareFeedModel moment) {
    return Container(
      child: Column(
        ...
        children: [
          Row(
            children: [
              ...
              CircularImage(imagePath: moment.imageDp ?? '', diameter: 35),
              horizontalGap(pagePadding / 2),
              Text(
                moment.name ?? '',
